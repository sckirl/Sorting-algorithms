...
class Algorithm(object):
    def __init__(self):
        ...
        self.start_icon = pygame.image.load('pygame7/Start-icon.png').convert_alpha()
        self.start_icon = pygame.transform.smoothscale(self.start_icon, (20, 20))
        self.target_icon = pygame.image.load('pygame7/Target-icon.png')
    ...
    def A_star(self):
        ...
        try:
            ...
            if len(self.open_list) > 0 and self.Astar_run:
                ...
                # generate children
                for new_position in [(0, -20), (0, 20), (-20, 0), (20, 0)]: # make plus/cross-shaped new positions
                    ...
                    # don't go outside the display size(win_x, win_y)
                    if new_child[0] > win_x - 20 or new_child[1] > win_y - 20 \
                            or new_child[0] < 0 or new_child[1] < 0: continue
                ...
                for child in self.children:
                    ...
                    nodes = [open_node.g for open_node in self.open_list]
    ...
    def Dijkstra(self):
        ...
        try:
            if self.Dijkstra_run:
                ...
                for positions in [(20, 0), (-20, 0), (0, 20), (0, -20)]: # make plus/cross-shaped new positions
                    new_nodes = (
                        (self.current_node.position[0] + positions[0], self.current_node.position[1] + positions[1])) # new nodes added from num in for loop

                    if new_nodes in self.walls: continue # make sure the terrain isn't a wall
                    if new_nodes[0] > win_x - 20 or new_nodes[1] > win_y - 20 or new_nodes[0] < 0 or new_nodes[1] < 0: continue # make sure it fits within the screen

                    children_positions = [i.position for i in self.children]
                    if new_nodes not in children_positions: # make sure to not add the same children twice
                        self.children.append(Node(self.current_node, new_nodes))

                for child in self.children:
                    child.value = abs(self.open_list[0].position[0] - child.position[0]) \
                                  + abs(self.open_list[0].position[1] - child.position[1]) # getting the value for each nodes
                    if child not in self.open_list and child not in self.closed_list:
                        self.open_list.append(child)
